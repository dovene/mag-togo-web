...
$fichier = basename($_FILES['image']['name'][0]);
$taille_maxi = 5000000;
$taille = filesize($_FILES['image']['tmp_name'][0]);
$extensions = array('.png', '.gif', '.jpg', '.jpeg', '.3gp', '.mp4');
$extension = strrchr($_FILES['image']['name'][0], '.'); 
...
{$data64 = base64_encode(file_get_contents($_FILES['image']['tmp_name'][0]));
     //On formate le nom du fichier ici...
     $fichier = strtr($fichier, 
          'ÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÒÓÔÕÖÙÚÛÜÝ� áâãäåçèéêëìíîïðòóôõöùúûüýÿ', 
          'AAAAAACEEEEIIIIOOOOOUUUUYaaaaaaceeeeiiiioooooouuuuyy');
     $fichier = preg_replace('/([^.a-z0-9]+)/i', '-', $fichier);
     if(move_uploaded_file($_FILES['image']['tmp_name'][0],$dossier.$fichier)) //Si la fonction renvoie TRUE, c'est que ça a fonctionné...
     {
...
//upload de la video 
 $dossierVideo='videos/';
$fichierVideo = basename($_FILES['image']['name'][1]);
$tailleVideo = filesize($_FILES['image']['tmp_name'][1]);
$extensionsVideo = array('.3gp', '.mp4');
$extensionsVideo = strrchr($_FILES['image']['name'][1], '.'); 
//Début des vérifications de sécurité...
if(!in_array($extension, $extensions)) //Si l'extension n'est pas dans le tableau
{
...
{$data64 = base64_encode(file_get_contents($_FILES['image']['tmp_name'][1]));
     //On formate le nom du fichier ici...
     $fichierVideo = strtr($fichierVideo, 
          'ÀÁÂÃÄÅÇÈÉÊËÌÍÎÏÒÓÔÕÖÙÚÛÜÝ� áâãäåçèéêëìíîïðòóôõöùúûüýÿ', 
          'AAAAAACEEEEIIIIOOOOOUUUUYaaaaaaceeeeiiiioooooouuuuyy');
     $fichierVideo = preg_replace('/([^.a-z0-9]+)/i', '-', $fichierVideo);
     if(move_uploaded_file($_FILES['image']['tmp_name'][1],$dossierVideo.$fichierVideo)) //Si la fonction renvoie TRUE, c'est que ça a fonctionné...
     {
